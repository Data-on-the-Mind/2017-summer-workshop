...
- $\bf{x_{\mbox{SATV}}}$ = vector os SATV scores of length $n$ (*independent variable* or *covariate*)
...
- $b_0$ and  $\beta_{\mbox{SATV}}$ are scalars (i.e., of length 1) and called *regression coefficients* or *parameters* ( $\sigma^2_{\epsilon}$ is also a parameter). $b_0$ is also known as the *intercept*.
...
| `~ x:y`|| Interaction between `x` and `y` (and no) |
...
It is important to always know of what type the variables are. Use `str()` on a `data.frame` to obtain information regarding the structure, including variable types: 
...
`model.matrix()` tranforms categorical covariates into numerical variables that can be used for fitting using a specific contrast function (see `?contr.sum`).
...
- Categorical variables are transformed into numerical variables using contrast functions (via `model.matrix()`).
...
- Type III tests require orthogonal contrasts (e.g.,`contr.sum`); recommended:
  ...
  - Complete cross-over interactions (i.e., main effects in presense of interaction) possible.
...
One can specify arbitray constrasts on marginal means (e.g., `contrast()`).
...
Statistical models defined by relationship of covariates and assumption of residual probability distribution. `formula` defines the relationship of covariates, `function` defines disributional assumption.
...
1. Identify probability distribution of data (or better: of residuals)
...
`afex` combines fitting (5.) and testing (6.):
...
- Our question: Does the Equation hold even if no apparent relationship between *A* and *B*? 
  - positive relavnce (PO): *A* is a reason for *B* 
...
- Each participant worked on 12 items, 4 per relevance condition.
- 2 x 2 between-subjects design:
  - DV: probability versus acceptability of 'if *A* then *B*'
  - Indicative conditional (if *A* then *B*) versus concessive conditional (even if *A* then *B*)
